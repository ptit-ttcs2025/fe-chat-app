...
export const userUri = {
  ...
  getUserById: (id: string) => `${URI}/users/${id}`,
};
...
export const userApis = {
  ...
  updateProfile: async (
    ...
    const response = await http.put<IUserProfile>(
      userUri.updateProfile(id),
      payload
    );
    return response.data;
  },
  ...
  uploadAvatar: async (
    ...
    const response = await http.post<IUploadAvatarResponse>(
      userUri.uploadImage,
      formData,
      {}
    );
...
/**
 * Hook lấy profile người dùng theo userId (khác current user)
 */
export const useGetUserById = (userId: string, enabled: boolean = true) => {
  return useQuery<IUserProfile, AxiosError>({
    queryKey: ["user", "by-id", userId],
    queryFn: async () => {
      const { data } = await http.get<IUserProfile>(
        userUri.getUserById(userId)
      );
      return data;
    },
